...
while(my $line=<F>){
	...
	for(my $i=$start_pos{$name}{'stop'}-750;$i<$start_pos{$name}{'stop'}+150;$i+=3){
		
		next if($i<$start_pos{$name}{'start'});
		next if($i>$start_pos{$name}{'tr_len'}-3);
		...
		my $relative_pos=$i-$start_pos{$name}{'stop'};
